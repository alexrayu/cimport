...
class Term {
  ...
  function __construct($path, $vocab) {
    $path = trim($path);
  ...
  protected function postProcessTerm($term) {
    ...
    $color = self::getColor(strtolower($term->name));
    if (!empty($color)) {
      $term->field_color['und'][0]['rgb'] = $color;
    }
  ...
  public static function getColor($colorname) {
    ...
    $hex = !empty($colors[$colorname]) ? $colors[$colorname] : NULL;
